...
<%
Dim qsearch__VarCateID
qsearch__VarCateID = "1"
if (Request("radiobutton")  <> "") then qsearch__VarCateID = Request("radiobutton") 
%>
<%
Dim qsearch__VarPname
qsearch__VarPname = "%"
if (Request("pname") <> "") then qsearch__VarPname = Request("pname")
%>
<%
Dim qsearch__VarAuthor
qsearch__VarAuthor = "%"
if (Request("tauthor") <> "") then qsearch__VarAuthor = Request("tauthor")
%>
<%
Dim qsearch__VarSupply
qsearch__VarSupply = "%"
if (Request("tsupply") <> "") then qsearch__VarSupply = Request("tsupply")
%>
<%
Dim qsearch__VarbeginMonth
qsearch__VarbeginMonth = "1"
if (Request("begin_month") <> "") then qsearch__VarbeginMonth = Request("begin_month")
%>
<%
Dim qsearch__VarbeginYear
qsearch__VarbeginYear = "40"
if (Request("begin_year") <> "") then qsearch__VarbeginYear = Request("begin_year")
%>
<%
Dim qsearch__VarendMonth
qsearch__VarendMonth = "12"
if (Request("end_month") <> "") then qsearch__VarendMonth = Request("end_month")
%>
<%
Dim qsearch__VarendYear
qsearch__VarendYear = "03"
if (Request("end_year") <> "") then qsearch__VarendYear = Request("end_year")
%>
<%
Dim qsearch__VarHotDeal
qsearch__VarHotDeal = "1"
if (Request("hotdeal_range") <> "") then qsearch__VarHotDeal = Request("hotdeal_range")
%>
<%
Dim qsearch__VarListPrice
qsearch__VarListPrice = "1"
if (Request("price_range") <> "") then qsearch__VarListPrice = Request("price_range")
%>
<%
Dim qsearch__VarStock
qsearch__VarStock = "1"
if (Request("instock") <> "") then qsearch__VarStock = Request("instock")
%>
<%
set qsearch = Server.CreateObject("ADODB.Recordset")
qsearch.ActiveConnection = MM_conneshop_STRING
qsearch.Source = "SELECT ProductID, ProductName, Supplier, Author, sImgUrl  FROM Products WHERE " + Replace(qsearch__VarCateID, "'", "''") + " AND ProductName LIKE '%" + Replace(qsearch__VarPname, "'", "''") + "%'AND Author LIKE '%" + Replace(qsearch__VarAuthor, "'", "''") + "%'AND Supplier LIKE '%" + Replace(qsearch__VarSupply, "'", "''") + "%'AND PubDate BETWEEN #" + Replace(qsearch__VarbeginMonth, "'", "''") + "/" + Replace(qsearch__VarbeginYear, "'", "''") + "# AND #" + Replace(qsearch__VarendMonth, "'", "''") + "/" + Replace(qsearch__VarendYear, "'", "''") + "# AND " + Replace(qsearch__VarHotDeal, "'", "''") + " AND " + Replace(qsearch__VarListPrice, "'", "''") + " AND " + Replace(qsearch__VarStock, "'", "''") + "  ORDER BY AddDate DESC"
...
<table width="760" border="0" cellspacing="0" cellpadding="0" align="center">
  <tr> 
    <td height="30" valign="middle"> 
      ...
      &nbsp; 共搜索到<%=(qsearch_total)%>个相关商品，当前为第<%=(qsearch_first)%> 到<%=(qsearch_last)%>个商品
