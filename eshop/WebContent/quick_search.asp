...
<%
Dim qsearch__varCategory
qsearch__varCategory = "1"
if (Request("mnuCategory")  <> "") then qsearch__varCategory = Request("mnuCategory") 
%>
<%
Dim qsearch__varProductName
qsearch__varProductName = "%"
if (Request("textPname") <> "") then qsearch__varProductName = Request("textPname")
%>
<%
set qsearch = Server.CreateObject("ADODB.Recordset")
qsearch.ActiveConnection = MM_conneshop_STRING
qsearch.Source = "SELECT ProductID, ProductName, Supplier, Author, sImgUrl  FROM Products  WHERE CategoryID =" + Replace(qsearch__varCategory, "'", "''") + " AND ProductName LIKE '%" + Replace(qsearch__varProductName, "'", "''") + "%'  ORDER BY AddDate DESC"
