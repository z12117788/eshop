...
public class VerifyCodeUtil {
	...
	private static final String VERIFCODE_KEY = "verifcode";
	...
	public static void buildVerifyCode(int verifySize,
			HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		// 1、生成随机验证码
		String verifcode = VerifyCodeUtil.generateVerifyCode(verifySize);
		// 2、将验证码保存到session中
		request.getSession().setAttribute(VERIFCODE_KEY, verifcode);
		// 3、将验证码写到图片上输出
		VerifyCodeUtil.outputImage(100, 40, response.getOutputStream(),
				verifcode);
	...
	public static boolean checkVerifyCode(HttpServletRequest request,
		...
		//从用户表单中提取输入的验证码值
		String verfcode2 = request.getParameter(paramName);
		
		//从Session中提取正确的验证码值
		String verfcode = (String) request.getSession().getAttribute(
				VERIFCODE_KEY);

		//相比较
		return null!=verfcode && verfcode.equalsIgnoreCase(verfcode2);
